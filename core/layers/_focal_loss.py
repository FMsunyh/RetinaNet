...
class FocalLoss(keras.layers.Layer):
	...
	#	a_x = core.backend.where(keras.backend.not_equal(labels, 0), keras.backend.ones_like(labels), keras.backend.zeros_like(labels))
	...
	#	a = core.backend.matmul(keras.backend.expand_dims(a_x, 0), a_y)
